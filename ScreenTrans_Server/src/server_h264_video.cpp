...
std::queue<ST::DecodedFrame> total_frames;

void Receive(Client& client, H264Decoder& decoder) {
    while (!glfwWindowShouldClose(window)) {
        int pk_size = client.ReceiveParseTo<int>();
        if (pk_size == 0) {
            continue;
        }
        ...
        for (int i = 0;i < pk_size;++i) {
            ...
            for (auto& frame : frames) {
                ...
                total_frames.push(std::move(frame));
            }
        }
    }
}
...
void Draw(H264Decoder& decoder) {
    ...
    while(!glfwWindowShouldClose(window)) {
        ...
        {
            std::lock_guard<std::mutex> lock(mtx_frames);
            if (total_frames.empty()) {
                continue;
            }

            frame = std::move(total_frames.front());
            total_frames.pop();
        }
...
int main() {
    ...
    ST::H264Decoder decoder;

	std::jthread receive_thread(Receive, std::ref(*client), std::ref(decoder));
    Draw(decoder);
